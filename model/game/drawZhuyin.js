...
function moveFinger(){
    ...
    let drawTime = [1500,2500,3600,4900,6100,7500,9000,10000,11000,12500];
    let strokeNumber = {b:8,4:8,5:8,6:8,7:6,f1:8}
        ...
        for(let i=1; i<strokeNumber[id];i++){
            setTimeout("document.querySelector('.moveFinger"+id+i+"').className = 'moveFinger"+id+(i+1)+"';", drawTime[i]);
            console.log(`moveFinger"+id+(i+1)+=${id+(i+1)}`);
        }
        setTimeout("document.querySelector('.moveFinger"+id+(strokeNumber[id])+"').className = 'finger"+id+"';", drawTime[strokeNumber[id]+1]);
        console.log(`drawTime[strokeNumber[id]+1]=${drawTime[strokeNumber[id]+1]}`)
        setTimeout("document.querySelector('.finger"+id+"').style.visibility='hidden';", (drawTime[strokeNumber[id]+1]+20));
